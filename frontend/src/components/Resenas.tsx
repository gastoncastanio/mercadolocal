...
export default function Resenas({ productoId, puedeResenar, ordenId }: ResenasProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-ml-line p-6">
      ...
      {/* Promedio */}
      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-ml-line2">
        <span className="text-4xl font-bold text-ml-ink">{promedio.toFixed(1)}</span>
        <div>
          <div className="text-2xl">{renderEstrellas(promedio)}</div>
          <p className="text-sm text-ml-muted">{resenas.length} resena{resenas.length !== 1 ? 's' : ''}</p>
        </div>
      </div>

      {/* Formulario */}
      {puedeResenar && (
        <form onSubmit={enviarResena} className="mb-6 pb-6 border-b border-ml-line2">
          <h4 className="font-semibold text-ml-ink mb-3">Deja tu resena</h4>
          <div className="mb-3">
            <label className="block text-sm text-ml-soft mb-1">Calificacion</label>
      ...
      {/* Lista de resenas */}
      {resenas.length === 0 ? (
        <p className="text-ml-muted text-center py-4">Aun no hay resenas</p>
      ) : (
        <div className="space-y-4">
          {resenas.map((r) => (
            <div key={r._id} className="border-b border-gray-50 pb-4 last:border-0">
              ...
              <p className="text-ml-soft text-sm">{r.comentario}</p>
              {r.respuestaVendedor && (
                <div className="mt-2 ml-4 pl-3 border-l-2 border-blue-200 bg-blue-50 rounded-r-lg py-2 px-3">
                  <p className="text-xs font-semibold text-ml-blue mb-1">Respuesta del vendedor</p>
                  <p className="text-sm text-ml-ink">{r.respuestaVendedor}</p>
                </div>
              )}
            </div>
          ))}
