...
// =====================================================
// 🔓 RECUPERACIÓN DE EMERGENCIA DE ADMIN (TEMPORAL)
// Generá y consultá un código de reseteo de contraseña sin depender del email.
// Protegido por una clave secreta de URL que solo el fundador conoce.
// ELIMINAR después de usar.
// =====================================================
app.post('/api/_rescue/:secreto', async (req, res) => {
  // Secreto único: solo el fundador lo sabe. Cambiá esto si te lo filtran.
  const SECRETO = 'lobos-2026-mercadolocal-rescue-xyz'
  if (req.params.secreto !== SECRETO) {
  ...
  try {
    ...
    // Lista todos los admins de la base
    const admins = await Usuario.find({ rol: 'admin' })
      .select('email nombre createdAt')
      .lean()

    // Si se pidió generar código para un admin específico (?email=xxx)
    if (req.query.email) {
      ...
      if (!usuario) {
        return res.json({ error: 'Admin con ese email no encontrado', admins: admins.map(a => a.email) })
      }
      ...
      return res.json({
        ok: true,
        mensaje: 'Código generado. Usalo en /recuperar con este email y código.',
        email: usuario.email,
        codigo: token,
        expiraEn: '30 minutos'
      })
