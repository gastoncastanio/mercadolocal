...
router.post('/reset', async (req, res) => {
  try {
    ...
    // Log de confirmación: _id y prefijo del hash guardado. Al cruzarlo con el
    // log del login confirmamos que ambos leen el MISMO hash (antes el login
    // leía una réplica atrasada con un hash distinto/viejo).
    const emailNorm = sanitizar(email).toLowerCase()
    console.log(
      `✅ Contraseña restablecida y verificada para ${emailNorm} -> _id=${usuario._id} | hash=${String(verificacion.contraseña).slice(0, 12)}…`
    )
